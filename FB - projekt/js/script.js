...
function visTidligereResultat() {
  ...
  const previousResultContainer = document.getElementById('previous-result-container');
  ...
  if (result) {
    ...
    if (previousResultContainer && previousResult) {
      previousResult.innerHTML = `Dit gemte resultat: <span class="result-type">${oversaetResultat(result)}</span>`;
...
function setupShowAnswersBtns() {
  document.querySelectorAll('.show-answers-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      const spmTekster = {
        post2: "Strandbillede: Ville du dele?",
        post3: "Barn græder: Ville du dele?",
        post4: "Nyfødt: Ville du dele?"
      };
      let html = "<ul>";
      for (let postId in spmTekster) {
        const svar = quizSvar[postId];
        let svarTekst = "Ikke besvaret";
        if (svar === "del") svarTekst = "<span style='color:green'>Jeg ville dele</span>";
        if (svar === "del_ikke") svarTekst = "<span style='color:red'>Jeg ville ikke dele</span>";
        html += `<li><b>${spmTekster[postId]}</b> ${svarTekst}</li>`;
      }
      html += "</ul>";
